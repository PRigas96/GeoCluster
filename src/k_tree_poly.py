import torch
import numpy as np
from src.models import Clustering, ClusteringLS,ClusteringPLS, Critic
...
class Ktree:
    ...
    class Node:
        ...
        def get_bounding_box(self):
            ...
            x = x[~torch.isinf(x)]
            y = y[~torch.isinf(y)]
            bounding_box = [[x.min().item(), x.max().item()], [y.min().item(), y.max().item()]]
        ...
        def create_critic(self, save_path_prefix="", plot=False):
            ...
            clustering = ClusteringPLS(train_data,
                                      n_of_centroids,
                                      dim)
